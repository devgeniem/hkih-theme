<?php
/**
 * Articles highlights ACF Flexible Content Layout
 */
...
/**
 * Class ArticleHighlightsLayout
 *
 * @package Geniem\Theme\ACF\Layouts
 */
class ArticleHighlightsLayout extends \Geniem\ACF\Field\Flexible\Layout {

    /**
     * Layout key
     */
    const KEY = '_article_highlights';
     ...
     * GraphQL Layout Key
     */
    const GRAPHQL_LAYOUT_KEY = 'LayoutArticleHighlights';
    ...
    public function __construct( $key ) {
        $label = __( 'Article highlights', 'hkih' );
        $key   = $key . self::KEY;
        ...
        $this->strings = [
            ...
            'offset'           => [
                'label'        => __( 'Article list offset', 'hkih-contact' ),
                'instructions' => 'For example: listing starts from the third newest article by setting this to 2',
            ],
        ];
    ...
    private function add_layout_fields() : void {
        ...
        try {
            ...
            $offset_field = ( new Field\Number( $this->strings['offset']['label'] ) )
                ->set_key( "{$key}_offset" )
                ->set_name( 'offset' )
                ->set_wrapper_width( 50 )
                ->set_instructions( $this->strings['offset']['instructions'] );
